...
CONTRACT_PATTERN = re.compile(
    r"\*\*\[([A-Z]+)\s+([\d.]+)\s+(P|C)\s+(\d{2}/\d{2}/\d{4})\s+\((\d+)\s*DTE\)",
    re.MULTILINE,
)
...
def parse_message(message_id: str, content: str, timestamp: datetime) -> OptionsFlowData | None:
    ...
    expiry = datetime.strptime(contract_match.group(4), "%m/%d/%Y").date()
    dte = int(contract_match.group(5))

    def extract_or_zero(pattern: re.Pattern[str], default: str = "0") -> str:
        match = pattern.search(content)
        return match.group(1) if match else default

    interval_volume_raw = extract_or_zero(INTERVAL_VOLUME_PATTERN)
    if interval_volume_raw == "0":
        interval_volume_raw = extract_or_zero(OVERALL_VOLUME_PATTERN)
    ...
    return OptionsFlowData(
        ...
        expiry=expiry,
        dte=dte,
        interval_volume=int(parse_number(interval_volume_raw)),
