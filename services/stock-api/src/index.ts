    ...
    return {
        ...
        paths: {
                    ...
                    parameters: [
                        {
                            ...
                            schema: { type: "string" },
                            description:
                                "Search query (ticker, name, or EXCHANGE: prefix)",
                            example: "WMT",
                        },
                        {
                            name: "type",
                ...
                get: {
                    ...
                    parameters: [
                        ...
                        },
                        {
                            ...
                            schema: { type: "string" },
                            description:
                                "JSON string of indicator options (inputs)",
                        },
                    ],
                    responses: {
                        200: {
            ...
            "/indicators": {
                get: {
                    summary:
                        "List all available indicator IDs and their descriptions",
                    responses: {
                        200: {
                ...
                IndicatorInfo: {
                    ...
                    properties: {
                        id: {
                            type: "string",
                            description:
                                "Indicator ID to use in /indicator endpoint",
                        },
                        name: {
                            type: "string",
...
fastify.get<{ Querystring: { symbol?: string } }>(
    ...
    async (request, reply) => {
        ...
        try {
            ...
            return reply
                .code(500)
                .send({ error: errorMessage(err, "Failed to fetch quote") });
        }
    },
);
...
fastify.get<{
    ...
    try {
        ...
        request.log.error(err);
        return reply
            .code(historyErrorStatus(err))
            .send({ error: errorMessage(err, "Failed to fetch history") });
    }
});
...
fastify.get<{
    ...
    try {
        ...
        return reply
            .code(500)
            .send({ error: errorMessage(err, "Failed to search markets") });
    }
});
...
fastify.get<{
    ...
    const { symbol, timeframe, range, to, indicatorId, options } =
        request.query;
    ...
    if (options) {
        try {
            ...
            return reply
                .code(400)
                .send({ error: "options must be valid JSON" });
        }
    }
    ...
    try {
        ...
        return reply
            .code(500)
            .send({ error: errorMessage(err, "Failed to fetch indicator") });
    }
});
